...
- `inputmode="numeric"` para abrir o teclado numérico do aparelho ao tocar no campo em dispositivos móveis
- `autocomplete="bday"` somente quando o campo representar data de nascimento; caso contrário, usar `autocomplete="off"` ou o valor contextual apropriado

Quando o campo usar `type="date"` e depender do seletor nativo do navegador, preservar esse comportamento. Para campos com máscara manual `dd/mm/aaaa`, preferir `type="text"` com `inputmode="numeric"`, sem remover o foco ao formatar.
...
Para campos de data, usar `inputmode="numeric"` para solicitar o teclado numérico no celular.
